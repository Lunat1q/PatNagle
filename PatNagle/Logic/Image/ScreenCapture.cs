...
namespace PatNagle.Logic.Image
{
    public static class ScreenCapture
    {
        ...
        /// <returns></returns>
        public static System.Drawing.Image CaptureScreenRelatively(double startX, double endX, double startY, double endY, bool fullScreen)
        {
            if (fullScreen)
            {
        ...
        /// <param name="endY">0-100 percent of end by Y</param>
        /// <returns></returns>
        internal static System.Drawing.Image CaptureWindow(IntPtr handle, double startX, double endX, double startY, double endY)
        {
            ...
            int sizeX = (int)((endX - startX) * width / 100);
            var sizeY = (int)((endY - startY) * height / 100);

            if (sizeX < 0 || sizeY < 0)
            {
                throw new ArgumentException($"Size: X: {sizeX} Y: {sizeY}");
            }
            IntPtr hBitmap = GDI32.CreateCompatibleBitmap(hdcSrc, sizeX, sizeY);
            // select the bitmap object
            IntPtr hOld = GDI32.SelectObject(hdcDest, hBitmap);
            // bitblt over
            int x = (int)(startX * width / 100);
            int y = (int)(startY * height / 100);
            GDI32.BitBlt(hdcDest, 0, 0, sizeX, sizeY, hdcSrc, x, y, GDI32.SRCCOPY);
            // restore selection
            ...
            //Debug.WriteLine($"size: {sizeX}-{sizeY}, coords: {x}-{y}, hdcSrc {hdcSrc}, {width}-{height}, bitmap:{hBitmap}");
        ...
        {
            System.Drawing.Image img = CaptureWindow(handle, 0, 100, 0, 100);
